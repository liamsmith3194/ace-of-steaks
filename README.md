...
- #### Colour Scheme
    ...
    -   The full length white navigation bar against the image but also black text and eye catching pseduo class styles.

    -   The gold is used throughout the site for th logo, all headings, subheadings and buttons.
    -   All main buttons in their native form are gold and black text.
    -   When hovered over the buttons reverse in style, black background and gold text.

    -   The navigation menu items are black, when hovered over the links change colour to gold and the font weight is raised to the thickness possible to ensure the user is certain where the cursor is positioned.
    -   The active class is styled using the hover class but underlined with a silver bar similar to the logo design. This make it even more clear for the user to know which page they are on.
...
- #### Typography
    -   All titles use the bold striking typeface of Bebas Neue all with various letter spacing differences between headings and subheadings to make them more legable.
    -   The body typeface of Montserrat produces a sharp contrast from the titles. This is a very popular font for web design because of it's readabilty.

- #### Imagery
    -   One image used throughout the site which sits behind the transparent body content window.
    -   It is quite a dark image to ensure the content is at the forefront the users attention and help the readabilty of the headings.
...
4. In GitPod set up a env.py file in the repository.
    -   Create an enviroment variable for "DATABASE_URL" and paste the value from Heroku.
    -   Create an enviroment variable for "SECRET_KEY" and create your key using any characters available.
